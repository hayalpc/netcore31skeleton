...
namespace NetCore31Skeleton.WebApi.Core.Utils
{
    public class HttpClientHelper<TRequest, TResponse> : IHttpClientHelper<TRequest, TResponse>
    {
        ...
        public HttpClientHelper(IHtttpClientCreator htttpClientCreator, ILogger logger, IMemoryCache _memCache)
        {
            this.logger = logger;
        ...
        public string LoadToken(string baseUrl, string method)
        {
            try
            {
                ...
                var bearerToken = streamTask.Result;
                if (bearerToken != null)
                {
                    _memCache.Set("BearerToken", bearerToken);
                    AddToken(bearerToken);
                }
                return bearerToken;
            }
            catch (Exception exp)
        ...
        public HttpResponseMessage UserValidate(string baseUrl, string method)
        {
            string uri = baseUrl + method;
